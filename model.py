...
class PositionalEncoding(nn.Module):
    def __init__(
        ...
        # size:[256]
        tmp = pow(10000.0, torch.arange(0, d_model, 2).float() / d_model)
...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    # 要为每一个q提供一份k，所以把第二维度扩展了q次，这样只有最后一列是True，正常来说最后一行也需要是True，
    # 但是由于作为padding的token对其他词的注意力不重要，所以可以这样写
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        # 这里其实第二个维度就是seq_len，多头是拆的hidden_size=n_heads*d_k=8*64=512
        # transpose本质上是取转置操作，让最后的矩阵，变成5x64，包含每个seq的64个维度
        Q = (
        ...
        # 4）o_proj部分,参照论文[batch_size,len_q,d_v]->[batch_size, len_q, d_model]
        output = self.o_proj(context)  # [batch_size, len_q, d_model]
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # 这是生成padding，参考self attention的mask，都是用的自身的input
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        # 解散batch，一个batch中有batch_size个句子，每个句子有tgt_len个词（即tgt_len行），现在让他们按行依次排布，
        # 如前tgt_len行是第一个句子的每个词的预测概率，再往下tgt_len行是第二个句子的，一直到batch_size * tgt_len行
        ret = dec_logits.view(-1, tgt_vocab_size)  #  [batch_size * tgt_len, tgt_vocab_size]
        return ret
